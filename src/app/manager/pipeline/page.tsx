...
export default function ManagerPipelinePage() {
  ...
  const updateStage = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['manager-deals'] }),
  })
  ...
  const handleDrop = (e: React.DragEvent, stage: DealStage) => {
    e.preventDefault()
    if (draggingDeal) { updateStage.mutate({ id: draggingDeal, stage }); setDraggingDeal(null) }
  }

  if (isLoading) return (
    <div>
      <PageHeader title="Pipeline" description="Team Kanban board" />
    ...
    <div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0">
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage} className="flex-shrink-0 w-72" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, stage)}>
              ...
              <div className="space-y-3 min-h-[200px]">
                {stageDeals.map((deal) => (
                  <div key={deal.id} draggable onDragStart={() => handleDragStart(deal.id)}
                    ...
                    {deal.properties?.title && <p className="text-xs text-muted-foreground mt-1.5 truncate">{deal.properties.title}</p>}
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-border">
                      <span className="text-xs font-semibold text-foreground">{formatCurrency(deal.value)}</span>
                      <span className="text-[10px] text-muted-foreground">{deal.expected_close ? new Date(deal.expected_close).toLocaleDateString() : 'No date'}</span>
                    </div>
                  </div>
                ))}
                {stageDeals.length === 0 && <div className="rounded-card border border-dashed border-border p-4 text-center"><p className="text-xs text-muted-foreground">Drop deals here</p></div>}
              </div>
            </div>
          )
        })}
